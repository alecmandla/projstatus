...
if want depth3; then
  ...
  row="$(printf '%s' "$out" | grep P0M1T1)"
  assert_contains "depth3 pointer: sub-issue count" "$row" "2/3"
  row="$(printf '%s' "$out" | grep P0M1T0)"
  assert_not_contains "depth3 pointer: done task has no sub count" "$row" "/"
